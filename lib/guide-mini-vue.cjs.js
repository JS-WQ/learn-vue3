...
function createRenderer(options) {
    ...
    //更新children，对比新旧节点
    function patchChildren(container, oldVnode, vnode, parentComponent, anchor) {
        ...
        if (newShapFlag & 4 /* ShapeFlags.TEXT_CHILDREN */) {
            ...
            if (oldShapFlag & 4 /* ShapeFlags.TEXT_CHILDREN */) {
                ...
                //老节点也是数组：双端对比diff算法
                patchKeyedChildren(oldVnode.children, vnode.children, container, parentComponent, anchor);
            }
        }
    }
    function patchKeyedChildren(oldChildren, newChildren, container, parentComponent, anchor) {
        let i = 0;
        ...
        if (i > e1) {
            ...
            if (i <= e2) {
                ...
                let nextPos = e2 + 1;
                let anchor = nextPos < newChildren.length ? newChildren[nextPos].el : null; //newChildren[nextPos].el保存的是oldChildren[]中的el
                while (i <= e2) {
            ...
            for (let index = s1; index <= e1; index++) {
                ...
                if (newIndex === undefined) {
                    ...
                    //该节点存在于新节点中
                    patch(oldChild, newChildren[newIndex], container, parentComponent, anchor);
                    patched++;
                }
            }
        }
